...
const NFT_WASM_FILEPATH: &str = "./../../../nft-tutorial/out/main.wasm";
...
// /home/raffland/backend-app/nft-tutorial/out/main.wasm
// /home/raffland/backend-app/workspace-test/examples/src/raffland_NFT.rs
#[tokio::main]
async fn main() -> anyhow::Result<()> {
    ...
    let raffland_wasm = std::fs::read(NFT_RAFFLAND)?;
    let raffland_contract = worker.dev_deploy(&raffland_wasm).await?;
    ...
    let request_payload = json!({
        "token_id": "1",
        "receiver_id": raffland_contract.id(),
        "metadata": {
    ...
    alice.call(&worker, &nft_contract.id(), "nft_mint")
    ...
    // alice.call(&worker, &nft_contract.id(), "nft_transfer")
    //     .args_json(request_payload)?
    //     .gas(DEFAULT_GAS as u64)
    //     .deposit(1)
    //     .transact()
    ...
    let resp: bool = alice
        .call(&worker, &raffland_contract.id(), "add_new_raffle")
        .args_json(request_payload1)?
        .gas(DEFAULT_GAS as u64)
        // .deposit(parse_near!("0.008 N"))
        .transact()
        .await?
    ...
    let tokens: serde_json::Value = owner
        .call(&worker, &nft_contract.id(), "nft_token")
        .args_json(serde_json::json!({"token_id": "1"}))?
        .transact()
        .await?
        .json()?;

    //extract owner_id from string which has json format
    let owner_id = tokens["owner_id"].as_str().unwrap();

    println!("nft: {:?}", owner_id);

    let result: u128 = owner
        .call(&worker, &raffland_contract.id(), "get_counter1")
        .view()
        .await?
        .json()?;
